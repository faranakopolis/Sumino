...
At first, I was controlling the request counts inside of each view but in order to make the system's performance better and make it scalable, I have decided to consider each blocking situation as a permission class to access a view, so it does not have to go through the view.
